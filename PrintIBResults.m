...
for i=1:length(Ec)
  for j=1:length(Elh)
    ...
    display(strcat(...
    'e',num2str(i),'-lh',num2str(j),' = ',num2str( EEc_hh(i,j) ,'%.3f' ),'eV = ',num2str(h*c/e/EEc_hh(i,j)*1e9,'%.0f'),'nm = '...
    ,num2str(EEc_hh(i,j)/h*e*1e-12,'%.0f'),'THz = ',num2str(EEc_hh(i,j)*e/h/c*1e-2,'%.0f'),'cm-1; |<e',num2str(i),'|lh',num2str(j),'>|='...
